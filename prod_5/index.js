...
const animateLoader = () => {
    ...
    gsap.timeline({defaults: {duration:1.8, ease:'expo.inOut'}}).to(loader.gallery, {scale: 1}).to(
        galleryItem,{
            width: '100vw',
        },
        0
        ...
        yPercent: -100,
    },0.2).to(loader.circleBottom,{
        yPercent: 100,
    },0.2);
};

const animateHero = () => {
    gsap.timeline({defaults: {duration:3.2, ease:'expo.inOut'}}).to([hero.headingTitle, hero.subheadingTitle], {yPercent: 0, rotate: 0}).to(
        [hero.headingTitleChars, hero.subheadingTitleChars], {yPercent: 0 , stagger : 0.05},
        0
    );
};
